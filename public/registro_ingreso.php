...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!Auth::verifyCsrfToken($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($errors)) {
            try {
                // Check if ingresos table exists, if not use gastos with negative amount
                $stmt = $conn->prepare(
                    "INSERT INTO ingresos_extra (concepto, categoria, monto, fecha_ingreso, sucursal_id, usuario_registro, comprobante, notas) 
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
                );
                $stmt->execute([
                    $concepto,
                ...
                logEvent('financiero', "Ingreso registrado: {$concepto} - \${$monto}", Auth::id(), null, $sucursal_id);
        ...
        <div class="mb-8 flex justify-between items-center">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">
                    <i class="fas fa-plus-circle text-green-600 mr-2"></i><?php echo $pageTitle; ?>
                </h1>
                <p class="text-gray-600">Registrar un nuevo movimiento de ingreso</p>
            </div>
            ...
            <div class="flex">
                ...
                <div class="ml-3">
                    ...
                    <div class="mt-2 text-sm text-blue-700">
                        <p>Los pagos de membresías se registran automáticamente en el módulo de Pagos.</p>
                        <p class="mt-1">Utilice este formulario solo para ingresos adicionales como ventas de productos, servicios extras, etc.</p>
                    </div>
                </div>
            </div>
        </div>
